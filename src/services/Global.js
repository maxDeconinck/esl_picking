...
class Global {
  ...
  static async prepareESL(pickingId, line, element, stock, complementInformation = '') {
    ...
    if(element.emplacement && element.emplacement.includes('FU.') && complementInformation) {
      ...
      if(maillons !== null && longueursBrin !== null) {
        stockDisplay = `${maillons[1]}*${longueursBrin[1]} x${line.nb_chaine}`; // Affichage du nombre de maillons et de brins à prélever
      }
    }
  ...
  **/
  ...
  static async refreshESL(deviceId) {
    ...
    if (!device.mac) {
      return ({ error: "Device has no MAC address" });
    }

    if (!device.fk_product && device.emplacement) {
      await Minew.addGoodsToStore({
      ...
      await Minew.changeTagDisplay(device.mac, {
        mode: "no_product",
      ...
      return ({ error: "Device is not associated with any product" });
    }
    ...
    if(!device.emplacement) {
      ...
      return ({ error: "Device has no associated emplacement" });
    }
    ...
    if (!stock || stock.length === 0) {
      return ({ error: "Stock information not found for the associated product and location" });
    }
    ...
    if(device.serial === 'serial'){
      ...
      numLot = await this.formatLots(stock.map(s => s.batch_number));
